...
class SwerveConstants {
  ...
  object AzimuthConstants {
    ...
    const val aziSubwooferLeft: Double =
        -30.0 // (Math.atan2(subwooferLeftX, subwooferSideY) * 180 / pi) + 180;

    const val aziSubwooferRight: Double =
        0.0 // (Math.atan2(subwooferRightX, subwooferSideY) * 180 / pi) + 180;

    const val aziSourceRed: Double = 60.0

    // -(Math.atan2(sourceXRed, sourceY) * 180 / pi) + 10;
    const val aziSourceBlue: Double =
  ...
  companion object {
    ...
    // Pose Estimation
    val kSwervePoseEstimatorStdData: List<TrainingDataPoint> =
        java.util.List.of(
            ...
            TrainingDataPoint(7.128826, 16.0333875, 30.8555875, 0.505372)) // 9 foot
