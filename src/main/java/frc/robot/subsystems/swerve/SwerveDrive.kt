...
class SwerveDrive(private val gyroIO: GyroIO, mod0: ModuleIO?, mod1: ModuleIO?, mod2: ModuleIO?, mod3: ModuleIO?) :
    ...
    @get:AutoLogOutput
    var chassisSpeeds: ChassisSpeeds = ChassisSpeeds()
     ...
     */
        ...
        swerveOdometry =
            SwerveDriveOdometry(
                SwerveConstants.swerveKinematics, gyroYaw, modulePositions
            )
    ...
    fun drive(
        translation: Translation2d, rotation: Double, fieldRelative: Boolean, isOpenLoop: Boolean
    ) {
        chassisSpeeds =
            if (fieldRelative
            ) ChassisSpeeds.fromFieldRelativeSpeeds(
                translation.x, translation.y, rotation, heading
            )
    ...
    val distanceToSpeaker: Unit
        get() {
            ...
            scoringLocation = if (ally.isPresent && ally.get() == Alliance.Red) {
    ...
    fun updatePose(gyroYaw: Rotation2d?, swerveModulePositions: Array<SwerveModulePosition?>?) {
        if (Robot.isSimulation()) {
            val fieldRelative =
                ChassisSpeeds.fromRobotRelativeSpeeds(chassisSpeeds, this.gyroYaw)
            simPose =
                simPose.transformBy(
                    Transform2d(
    ...
    fun zeroHeading() {
        if (FeatureFlags.kSwerveUseVisionForPoseEst) {
            poseEstimator.resetPosition(
                gyroYaw,
                modulePositions,
                Pose2d(pose!!.translation, Rotation2d())
            )
        } else {
    ...
    var heading: Rotation2d?
        ...
        set(heading) {
            if (FeatureFlags.kSwerveUseVisionForPoseEst) {
                poseEstimator.resetPosition(
                    gyroYaw, modulePositions, Pose2d(pose!!.translation, heading)
                )
            } else {
                swerveOdometry.resetPosition(
                    gyroYaw, modulePositions, Pose2d(pose!!.translation, heading)
    ...
    val gyroYaw: Rotation2d
        // @Log(name = "Gyro Yaw")
        get() {
            val yaw = if (Robot.isReal()) gyroInputs.yawPosition
            else simPose.rotation
            return yaw
        }
    ...
    fun configureAutoBuilder(
        ...
        AutoBuilder.configureHolonomic(
            ...
            HolonomicPathFollowerConfig(
                PIDConstants(
                    translationKP, translationKI, translationKD, 0.0
    ...
    val isAlignedToSpeaker: Boolean
        get() {
            ...
            val alliance = DriverStation.getAlliance()
            if (alliance.isPresent) {
                val speakerPose = if (alliance.get() == Alliance.Red) {
                    // Pose of speaker
                    AutoConstants.kRedSpeakerLocation
                } else {
                    // Pose of speaker
                ...
                val distanceToSpeaker =
                    currentPose.translation.getDistance(speakerPose.translation)
    ...
    companion object {
        ...
        init {
            if (FeatureFlags.kLocalizationStdDistanceBased) {
                ...
                val trainStdDevXTranslation =
                    DoubleArray(SwerveConstants.kSwervePoseEstimatorStdData.size)
                val trainStdDevYTranslation =
                    DoubleArray(SwerveConstants.kSwervePoseEstimatorStdData.size)
